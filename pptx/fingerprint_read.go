...
// ReadPresentationTemplateHash reads the template hash embedded in the
// ppt/slidownMeta entry of an existing .pptx file by slidown. Returns "" when
// the entry is absent (file was not generated by this version of slidown, or was
// re-saved by an external tool that dropped the entry).
func ReadPresentationTemplateHash(pptxPath string) (string, error) {
	parts, _, err := readZipPartsFromPath(pptxPath)
	if err != nil {
		return "", err
	}
	return strings.TrimSpace(string(parts["ppt/slidownMeta"])), nil
}

// ReadPresentationTemplateHashFromBytes reads the template hash from in-memory
// .pptx bytes. Returns "" on any error or if the entry is absent.
